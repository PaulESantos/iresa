...
#' @param year Año para el cual calcular el ranking. Por defecto 2025.
#' @param compare_with_2019 Lógico. Si es TRUE, calcula cambios respecto a 2019.
#'   Por defecto TRUE.
#' @param ascending Lógico. Si es TRUE, valores más bajos obtienen mejor ranking
...
calcular_ranking_indicador <- function(data,
  ...
  if (!year %in% c(2019, 2024, 2025)) {
    stop("El año debe ser 2019, 2024 o 2025", call. = FALSE)
  }
  ...
  # Detectar columnas de agrupación (indicador o indice_pilar)
  ...
  # PASO 2: Construir el resultado final según los parámetros
  ...
  if (compare_with_2019 && year != 2019) {
    ...
    if (!"score_2019" %in% names(resultado)) {
      warning("No se encontró 'score_2019'. Se omitirá la comparación.", call. = FALSE)
    ...
    # Calcular cambios entre año actual y 2019
    resultado <- resultado |>
      dplyr::mutate(
...
#' del IRESA de forma automática, detectando la dirección correcta del ranking.
#'
...
#'
#' @details
#' La función detecta automáticamente qué indicadores requieren ranking ascendente
#' (valores bajos = mejor) basándose en una lista predefinida de indicadores.
#'
...
calcular_rankings_completo <- function(data,
  ...
  indicadores_ascendentes <- c(
    "Calidad del aire",
    "Generación total de residuos sólidos municipales ajustados por PBI",
    "Degradación de ecosistemas",
    "Conflictos socioambientales activos",
    "Delitos denunciados por minería ilegal",
    "Agua no facturada",
    "Viviendas informales",
    "Pérdidas por incendios forestales"
  )

  # Detectar columna de agrupación
...
#' Obtener Ranking de una Región Específica
#'
#' Extrae el ranking de una región particular para uno o todos los indicadores.
#'
#' @param data Un data frame del IRESA con rankings calculados.
#' @param region Nombre de la región (ej: "Tacna", "Lima*", "Loreto").
#' @param year Año del ranking. Por defecto 2025.
#'
#' @return Un tibble con el ranking de la región especificada.
#'
#' @examples
#' \dontrun{
...
obtener_ranking_region <- function(data, region, year = 2025) {
  ...
  if (!rank_col %in% names(data)) {
    stop(glue::glue("No se encontró la columna '{rank_col}'. Primero calcule los rankings con calcular_ranking_indicador() o calcular_rankings_completo()."),
         call. = FALSE)
  }

  if (!region %in% data$region) {
    regiones_disponibles <- unique(data$region)
    stop(glue::glue("La región '{region}' no se encontró en los datos.\nRegiones disponibles: {paste(regiones_disponibles, collapse = ', ')}"),
         call. = FALSE)
  }
...
#'
#' Identifica regiones que comparten la misma posición en el ranking después
#' del recalculo.
#'
#' @param data Un data frame del IRESA con rankings calculados.
#' @param year Año del ranking a analizar. Por defecto 2025.
#'
...
#'
#' # Ver solo empates en un indicador específico
#' empates_ca |>
#'   filter(indicador == "Acceso a agua segura")
#' }
#'
...
identificar_empates <- function(data, year = 2025) {
  ...
  if (!rank_col %in% names(data)) {
    stop(glue::glue("No se encontró la columna '{rank_col}'"), call. = FALSE)
  }
  ...
  if (nrow(resultado) == 0) {
    message(glue::glue("No se encontraron empates en el ranking del año {year}"))
...
#' - No hay saltos mayores a 1 + número de empates
#' - Empates tienen el mismo score
#' - Rankings están en el rango correcto [1, n_regiones]
#'
#' @param data Un data frame con rankings calculados.
#' @param year Año a validar. Por defecto 2025.
#'
#' @return Lista con:
#'   - `valido`: Lógico indicando si pasa todas las validaciones
#'   - `errores`: Vector con mensajes de error (si hay problemas)
#'   - `detalles`: Tibble con estadísticas del ranking
#'
#' @examples
#' \dontrun{
#' ranking <- calcular_ranking_indicador(
...
validar_ranking <- function(data, year = 2025) {
  ...
  if (!rank_col %in% names(data)) {
    errores <- c(errores, glue::glue("No se encontró la columna '{rank_col}'"))
  ...
  # Validación 1: Rankings deben empezar en 1
  ...
  # Validación 2: Empates deben tener el mismo score
  ...
  # Validación 3: No debe haber NAs en rankings
